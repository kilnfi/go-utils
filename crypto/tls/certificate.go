...
func X509KeyPair(certPEMBlock, keyPEMBlock []byte) (tls.Certificate, error) {
	...
	if len(certPEMBlock) > 0 {
		cert.Certificate, err = DecodePEM(certPEMBlock, "CERTIFICATE")
		if err != nil {
	...
	if len(keyPEMBlock) > 0 {
		var keys [][]byte
		keys, err = DecodePEM(keyPEMBlock, "PRIVATE KEY")
		if err != nil {
			return tls.Certificate{}, err
		}
...
func DecodePEM(block []byte, typ string) ([][]byte, error) {
	...
	if certs, err = decodePEM(block, typ); err == nil {
		return certs, nil
	}
	rErr := err

	// Parses assuming block headers are missing
	if certs, err = decodePEM([]byte(fmt.Sprintf("-----BEGIN %v-----\n%v\n-----END %v-----", typ, string(block), typ)), typ); err == nil {
		return certs, nil
	}
...
func decodePEM(raw []byte, typ string) ([][]byte, error) {
	var (
		blocks            [][]byte
...
func (pair *CertificateFileKeyPair) Certificate() (tls.Certificate, error) {
	pair.loadOnce.Do(func() {
		var cert, key []byte
		cert, pair.err = os.ReadFile(pair.CertPath)
		if pair.err != nil {
			return
		}

		key, pair.err = os.ReadFile(pair.KeyPath)
		if pair.err != nil {
			return
		}

		pair.cert, pair.err = X509KeyPair(cert, key)
	})

	return pair.cert, pair.err
}
...
func (pair *CertificateFileCA) Certificate() (tls.Certificate, error) {
	pair.loadOnce.Do(func() {
		var cert []byte
		cert, pair.err = os.ReadFile(pair.Path)
		if pair.err != nil {
			return
		}

		pair.cert, pair.err = X509KeyPair(cert, nil)
	})
